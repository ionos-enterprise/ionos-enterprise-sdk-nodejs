...
module.exports={
	...
	listLans : function(dc_id,callback){
		pbreq.is_get([ "datacenters",dc_id,"lans"  ],callback)
	},

	createLan : function(dc_id,jason,callback){
		pbreq.is_post([ "datacenters",dc_id,"lans"  ],jason,callback)
	},

	getLan : function(dc_id,lan_id,callback){
		pbreq.is_get([ "datacenters",dc_id,"lans",lan_id  ],callback)
	},

	updateLan : function(dc_id,lan_id,jason,callback){
		pbreq.is_put([ "datacenters",dc_id,"lans",lan_id  ],jason,callback)
	},

	patchLan : function(dc_id,lan_id,jason,callback){
		pbreq.is_patch([ "datacenters",dc_id,"lans",lan_id  ],jason,callback)
	},

	deleteLan : function(dc_id,lan_id,callback){
		pbreq.is_del([ "datacenters",dc_id,"lans",lan_id  ],callback)
	},

	listLanMembers : function(dc_id,lan_id,callback){
		pbreq.is_get([ "datacenters",dc_id,"lans",lan_id,"nics"  ],callback)
	}
