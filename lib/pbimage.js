//		Image.js

module.exports={
/**
Image
Images collection
	List images
Image
	Retrieve an image
	Replace properties of an image
	Partially update an image
	Remove an image
	
	Complete
	
**/
	listImages : function(callback){
		pbreq.is_get([ "images"  ],callback)
	},

	getImage : function(image_id,callback){
		pbreq.is_get([ "images",image_id  ],callback)
	},

	updateImage : function(image_id,jason,callback){
		pbreq.is_put([ "images",image_id  ],jason,callback)
	},

	patchImage : function(image_id,jason,callback){
		pbreq.is_patch([ "images",image_id  ],jason,callback)
	},

	deleteImage : function(image_id,callback){
		pbreq.is_del([ "images",image_id  ],callback)
	}

}