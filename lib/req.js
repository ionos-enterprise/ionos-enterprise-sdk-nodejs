...
module.exports = {
    ...
    fullheader: function () {
        ...
        req.options.headers['User-Agent'] = 'ionosenterprise-sdk-nodejs/5.1.0'
    ...
    mk_url: function (aray, callback) {
        ...
        if (!req.options.headers['Authorization']) {
            if (process.env.IONOS_USERNAME && process.env.IONOS_PASSWORD) {
                req.options.headers['Authorization'] = 'Basic ' + new Buffer(process.env.IONOS_USERNAME + ':' + process.env.IONOS_PASSWORD).toString('base64');
            }
        }
